...
def copy_function(addr):
    func_addr = idc.get_name_ea_simple(idaapi.get_func_name(addr))
...
def get_cursor_func_ref():
    ...
    tform_type     = idaapi.get_widget_type(current_tform)

    # get the hexrays vdui (if available)
    vu = idaapi.get_widget_vdui(current_tform)
    if vu:
        cursor_addr = vu.item.get_ea()
    elif tform_type == idaapi.BWN_DISASM:
        cursor_addr = idaapi.get_screen_ea()

        op_addr = idc.get_operand_value(cursor_addr, idaapi.get_opnum())
...
def get_selected_funcs():
    tform = idaapi.find_widget("Functions window")
    if not tform:
